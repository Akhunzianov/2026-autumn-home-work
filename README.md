...
**ВНИМАНИЕ!** Нарушение следующих правил повлечёт снижение баллов (вплоть до 0), будьте аккуратны
...
* Не добавлять зависимости. Все задания возможно выполнить с текущим списком зависимостей. Возможно список зависимостей будет расширяться для новых заданий. 
* Не копировать чужой код, даже частично
* Не использовать большие языковые модели (БЯМ, LLM, ChatGPT, Sonnet, DeepSeek, GLM и т.д.) для написания кода

# Правила оценки заданий

* Чтобы задание было засчитано - необходимо (но не достаточно) чтобы были пройдены все соответствующие тесты и стилистические (code style) проверки
* Задание с падающими тестами или др. проверками - не будет оцениваться. Это ваша ответственность убедиться, что ваш PR "зелёный", никто не должен вам об этом напоминать
* Некоторые задания не имеют тестов и проверяются исключительно по коду
* За каждое засчитанное задание оценка может быть меньше возможной (но не больше), если оценивающий сочтёт нужным снять баллы
* Примеры, за что могут снять баллы - много ошибок в решении (много комментов на ревью), решение излишне переусложнено, решение проходит тесты, но в реальности не работает на других валидных входных данных (не корректное решение)  
* Оценивающие не собираются "жестить" и "заваливать" - это не цель заданий. Корректно сделанное решение с большой вероятностью получит полное кол-во баллов
